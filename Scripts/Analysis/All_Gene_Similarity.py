#!/usr/bin/env python
"""Script to generate alignments for all pairwise genes in the genome. Woof.
Translates CDS, aligns with clutal-omega, then back-translates. Takes three
arguments:
    1) List of gene IDs
...
if __name__ == '__main__':
    ...
    outdir = sys.argv[3]
    gids = [g.strip() for g in open(gene_ids, 'r').readlines()]
    tx_seqs = parse_transcripts(transcripts)
    # Build pairwise combinations of transcript IDs to align, use as args
    worker_args = []
    for t1, t2 in itertools.combinations(gids, 2):
        worker_args.append((t1, t2, tx_seqs, outdir))
