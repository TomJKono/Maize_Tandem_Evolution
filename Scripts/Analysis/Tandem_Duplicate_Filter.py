...
def main(tandem, gff_file, cds, te_db):
    ...
    with open(tandem, 'r') as f:
        for line in f:
            tmp = line.strip().split()
            tand_ids = sorted(tmp[1].split(','))
            num_exons = [len(gff_data[g]['Exons']) for g in tand_ids]
            lengths = ','.join([str(gff_data[g]['Length']) for g in tand_ids])
            #   Get which genes have TE sequence similarity
            te_genes = []
            te_names = []
            for geneid in tand_ids:
            ...
            l_wote = [str(gff_data[g]['Length']) for g in tand_ids if g not in te_genes]
            if len(te_names) == 0:
                te_pres = 'NA'
            else:
            ...
            if len(l_wote) == 0:
                ...
                lenwote = ','.join(l_wote)
            cluster = ','.join(tand_ids)
            if len(te_genes) == 0:
                withte = 'NA'
            else:
                withte = ','.join(te_genes)
            nexons = ','.join([str(s) for s in num_exons])
            nexonswote = [str(x[1]) for x in zip(tand_ids, num_exons) if x[0] not in te_genes]
